...
const getUnit = (productData: Product | null) => {
  ...
  if (name.includes('water') || name.includes('juice') || name.includes('milk') || name.includes('cola') || name.includes('soda') || name.includes('oil')) return '/ Bottle';
...
onMounted(async () => {
  try {
    ...
    // 1. Same Strict Logic as Home.vue
...
<template>
  <div class="max-w-5xl mx-auto px-4 py-12">
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center min-h-[50vh] gap-4 animate-pulse">
      ...
      <h3 class="text-xl font-bold text-gray-700 dark:text-gray-300 tracking-wide">Inspecting Item Details...</h3>
    </div>
    ...
    <div v-else-if="product" class="grid grid-cols-1 md:grid-cols-2 gap-12 bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
      ...
      <div class="flex flex-col justify-center">
        <span class="text-xs uppercase font-bold tracking-widest text-blue-600 dark:text-blue-400 mb-2">{{ product.category }}</span>
        ...
        <button @click="cartStore.addToCart(product)" class="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white font-bold px-8 py-3.5 rounded-xl transition-colors shadow-sm focus:outline-none focus:ring-4 focus:ring-blue-100 dark:focus:ring-blue-900">
          Add to Shopping Cart
        </button>
      </div>
    </div>

    <div v-else class="text-center text-red-500 py-12">Product details could not be found.</div>
